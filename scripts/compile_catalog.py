...
def build_artifacts(
    ...
    dependency_template = "\n".join(
        [
            ...
            "{%- endfor -%}",
            "{%- endfor %}",
            "{%- if ai_capabilities in ['agents', 'rag', 'inference'] or 'supabase' in [sql_store, document_store] or auth | default('none', true) in ['api-key', 'supabase-auth'] %}",
            '    "pydantic>=2.13.4,<3",',
            "{%- endif %}",
            "{%- if ai_capabilities in ['agents', 'rag', 'inference', 'training'] or sql_store != 'none' or document_store != 'none' or vector_store != 'none' or graph_store != 'none' or cache_store != 'none' or auth | default('none', true) != 'none' %}",
